			...
			try {
				...
				if (len > 0) {
					...
					if (notifheader != null && notifdescription != null) {
						...
						notifheader = filter(notifheader.substring(2));

						notifdescription = filter(notifdescription.substring(2));

						BASE64Decoder decoder = new BASE64Decoder();
						byte[] decodedBytes = decoder.decodeBuffer(notifheader);

						notifheader = new String(decodedBytes, "UTF-8");

						decodedBytes = decoder.decodeBuffer(notifdescription);

						notifdescription = new String(decodedBytes, "UTF-8");
